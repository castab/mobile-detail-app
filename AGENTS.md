...
│   │   ├── components/         # One file per component (Button, Badge, Card, Header, etc.)
│   │   └── config/             # Tests for config/app.ts
...
- **Vitest** handles unit and component tests — fast, isolated, no browser required. API is nearly identical to Jest so the mental model is familiar.
- **Playwright** handles end-to-end tests — runs a real browser, interacts with the app the way a real user would on a real device.
- They complement each other. Vitest catches broken logic and components in milliseconds. Playwright catches broken experiences that only show up in a real browser.
- In this phase there is no business logic to unit test, so the emphasis is on getting the infrastructure set up correctly and writing meaningful Playwright tests that verify the UI actually works.

---

### Vitest Setup

Install: `vitest`, `@vitejs/plugin-react`, `@testing-library/react`, `@testing-library/user-event`, `@testing-library/jest-dom`, `jsdom`.
...
### Vitest Tests (Phase 1)

#### `tests/unit/config/app.spec.ts`
- `appConfig.name` falls back to `'Fresh Finish'` when `NEXT_PUBLIC_APP_NAME` is not set.
- `appConfig.services` contains exactly two entries.
- Each service has `id`, `name`, `category`, `description`, and `duration` defined.
- `appConfig.nav` contains entries for `/services` and `/book`.

#### `tests/unit/components/Button.spec.tsx`
- Renders a `<button>` element by default.
- Renders a `<a>` element when `href` is provided.
- Applies the correct classes for each `variant` (`primary`, `ghost`, `danger`).
- Applies the correct classes for each `size` (`default`, `sm`).
- Is keyboard focusable.
- Does not fire `onClick` when `disabled`.

#### `tests/unit/components/Badge.spec.tsx`
- Renders with the correct text content.
- Applies the correct classes for each `variant`.

#### `tests/unit/components/Card.spec.tsx`
...
Install: `@playwright/test`. Run `npx playwright install` to download browser binaries (Chromium, Firefox, WebKit).
...
### Playwright Tests (Phase 1)

All tests run across the three configured projects: Desktop Chrome, iPhone 14, and Pixel 7. A test that passes on desktop but fails on mobile is a failing test.

#### `tests/e2e/homepage.spec.ts`
- Page loads with a `200` status.
- The hero heading is visible and contains the app tagline.
- The "Book Now" CTA is visible and links to `/book`.
- The "Our Services" CTA is visible and links to `/services`.
- The Services Preview section shows exactly two service cards.
- The How It Works section is visible with three steps.
- The Trust Bar is visible.
- The Footer is visible and contains the app name.

#### `tests/e2e/services.spec.ts`
- Page loads with a `200` status.
- The page heading "Our Services" is visible.
- Exactly two service detail cards are rendered.
- Each card contains a "Book This Service" link that points to `/book`.
- The bottom CTA section is visible.

#### `tests/e2e/booking.spec.ts`
- Page loads with a `200` status.
- All three steps are visible on the page (Step 1, Step 2, Step 3).
- The first service card has a visible selected state.
- The calendar is visible.
- At least one time slot button is visible.
- At least one time slot button is in a disabled/unavailable state.
- All form fields (Name, Email, Phone, Address, Vehicle Type, Notes) are present.
- The "Confirm Booking" button is visible.
- Clicking "Confirm Booking" does not navigate away (no submit handler in this phase).

#### `tests/e2e/nav.spec.ts`

**Desktop:**
- The header is visible on all three pages.
- Nav links "Services" and "Book Now" are visible in the header.
- Clicking "Services" navigates to `/services`.
- Clicking "Book Now" navigates to `/book`.

**Mobile (iPhone 14 + Pixel 7 only):**
- Desktop nav links are not visible at mobile viewport.
- A hamburger/menu button is visible in the header.
- Tapping the menu button opens the nav drawer.
- The nav drawer contains "Services" and "Book Now" links.
...
# Unit tests with visual UI
npm run test:ui

# E2E tests (headless)
npm run test:e2e

# E2E tests with browser UI (useful for debugging)
npm run test:e2e:ui
```

> **Note**: Playwright's `webServer` config will automatically start `npm run dev` before running e2e tests if the dev server isn't already running. In Phase 1 this starts Next.js directly — no database required.
...
10. All Vitest unit tests pass (`npm test`).
11. All Playwright e2e tests pass on Desktop Chrome, iPhone 14, and Pixel 7 (`npm run test:e2e`).
...
- New Vitest tests: availability logic, Zod schema validation, booking form validation.
- New Playwright tests: full booking flow submission, form validation errors, calendar date selection.
